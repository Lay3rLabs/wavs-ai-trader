...
use crate::{
    ...
    core::{calculate_weights, TradeStrategy},
};
...
impl Guest for Component {
    fn run(action: TriggerAction) -> std::result::Result<Option<WasmResponse>, String> {
        match action.data {
            TriggerData::Cron(TriggerDataCron { trigger_time }) => block_on(async move {
                ...
                let payload =
                    calculate_weights(query_client, address, trade_strategy, trigger_time.nanos)
                        .await
