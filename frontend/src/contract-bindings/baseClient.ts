...
} from "interchainjs/cosmwasm/wasm/v1/query";
...
const toUint8Array = (obj: unknown): Uint8Array => {
...
export const calculateGasFromChain = (
  ...
  try {
    ...
    if (feeTokens && feeTokens.length > 0) {
      ...
      const gasPrice =
        primaryToken.averageGasPrice ||
        primaryToken.lowGasPrice ||
        primaryToken.fixedMinGasPrice ||
        0.025;
...
export interface ICosmWasmClient {
  queryContractSmart(contractAddr: string, query: unknown): Promise<unknown>;
}
...
export function getCosmWasmClient(rpcEndpoint: string): ICosmWasmClient {
  return {
    queryContractSmart: async (contractAddr: string, query: unknown) => {
      ...
      const response: QuerySmartContractStateResponse =
        await getSmartContractState(rpcEndpoint, request);
...
export function getSigningCosmWasmClient(
  ...
  return {
    queryContractSmart: async (contractAddr: string, query: unknown) => {
      if (!rpcEndpoint) {
        throw new Error(
          "rpcEndpoint is required for queryContractSmart in signing client",
        );
      }
      const request: QuerySmartContractStateRequest = {
        address: contractAddr,
