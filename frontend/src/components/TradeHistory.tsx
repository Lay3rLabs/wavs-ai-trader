...
export function TradeHistory() {
  ...
  const allDenoms = events.flatMap((event) => {
    const denoms: string[] = [];
    // Check for denom values in the data object
    Object.entries(event.data).forEach(([key, value]) => {
      // Look for keys that contain denoms (like fromDenom, toDenom, denom)
      if (key.toLowerCase().includes("denom") && typeof value === "string") {
        denoms.push(value);
      }
      // Also check if the value itself looks like a denom
      if (
        typeof value === "string" &&
        (value.startsWith("u") ||
          value.startsWith("ibc/") ||
  ...
  const getEventDescription = (event: (typeof events)[0]) => {
    ...
    switch (event.type) {
      ...
      case "rebalancing":
        // New format: trade data with from/to amounts and denoms
        if (
          data.fromAmount &&
          data.toAmount &&
          data.fromDenom &&
          ...
          const fromMeta = tokenMetadata.get(data.fromDenom);
          const toMeta = tokenMetadata.get(data.toDenom);

          const fromSymbol =
            fromMeta?.symbol || data.fromDenom.replace("u", "").toUpperCase();
          const toSymbol =
            toMeta?.symbol || data.toDenom.replace("u", "").toUpperCase();

          console.log("META", data.fromDenom, tokenMetadata);
          ...
          return `Swapped ${fromAmount} ${fromSymbol} → ${toAmount} ${toSymbol}`;
        }
        // Legacy format
        if (data.swap_count) {
      ...
      case "price_update":
        if (data.denom && data.price_usd) {
          ...
          const symbol =
            meta?.symbol || data.denom.replace("u", "").toUpperCase();
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-yellow-700 dark:text-yellow-300">
                    {error}. The vault is still functional - only historical
                    data display is affected.
                  </p>
                </div>
              </div>
            </div>
          ...
          {!isLoading && events.length === 0 && !error && (
            <div className="rounded-lg border border-zinc-200 bg-white p-12 text-center dark:border-zinc-800 dark:bg-black">
              ...
              <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
                Vault activity will appear here once deposits or trades are
                made.
              </p>
